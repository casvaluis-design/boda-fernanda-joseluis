...
function WABtn({ phone, name, link }: { phone?: string; name: string; link: string }) {
  ...
  const msgInvitacion = encodeURIComponent(`Hola, ${name} ¡Por fin llegó el momento! ${e(0x1F942)}${e(0x2728)}\n\nNos hace la ilusión más grande del mundo compartir contigo nuestra invitación oficial. Hemos planeado este día con muchísimo amor y nuestra boda no sería lo mismo sin ti.\n\nToda la información del evento la encontrarás detallada dentro del siguiente enlace:\n\n${link}\n\nSolo te pedimos un enorme favor: ayúdanos confirmando tu asistencia como máximo el 30 de agosto para poder asegurar tu lugar.\n\n¡Que empiece la cuenta regresiva! Nos morimos de ganas de celebrar contigo. ${e(0x1F389)}${e(0x1F57A)}`);

  const msgRecordatorio = encodeURIComponent(`¡Hola, ${name}! ${e(0x1F90D)}\n\nFernanda y Jose Luis aquí — te escribimos porque aún no hemos recibido tu confirmación y nos encantaría tenerte con nosotros el día más especial de nuestras vidas.\n\nSabemos que el tiempo vuela, así que te pedimos de la manera más cariñosa que confirmes tu asistencia a través de tu enlace personal antes del 30 de agosto:\n\n${link}\n\nSolo te toma un momento y nos ayuda muchísimo para organizar todo con tiempo. ${e(0x1F64F)}\n\nY si tienes alguna duda sobre el hospedaje o cualquier otra cosa, escríbenos con toda confianza, con gusto te ayudamos. ${e(0x1F4AC)}\n\n¡Esperamos verte pronto! ${e(0x1F942)}\nFer y Luis`);

  const waUrl = `https://wa.me/${phone.replace(/\D/g, "")}`;

  return (
    <div className="flex gap-1">
      <a href={`${waUrl}?text=${msgInvitacion}`} target="_blank" rel="noopener noreferrer"
      ...
      <a href={`${waUrl}?text=${msgRecordatorio}`} target="_blank" rel="noopener noreferrer"
        title="Enviar recordatorio"
        className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded transition-opacity hover:opacity-75"
        style={{ background: "#128C7E", color: "white", fontFamily: "var(--font-jost)", whiteSpace: "nowrap" }}>
        {WA_ICON} Recordatorio
      </a>
    </div>
  );
}
